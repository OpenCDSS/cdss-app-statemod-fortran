c outplnw - prints for every Aug plan the well pumping associated with that plan
...
c
      subroutine OutPlnW
c
c       OutPlnW; It prints for every Aug plan the well pumping 
c		 associated with that plan
c
...
c
c		cwellP(i) = augmentation plan ID
...
c
      include 'common.inc'
c                           
      dimension   cwellP(10), nwellW(maxdvrw), nwellP(maxdvrw),
...
c
c       write(nlog,*) '  OutPlnW; iid, idallx ',  iid, idallx
      endif
c
c
c
...
c     write(nlog,*) '  OutPlnW; idallx, nid = ', idallx, nid
c                      
      ida0 = '0           '
      ip1 = 0                                         
      ix = 25
...
c  
c _________________________________________________________
c		Step 10; Get every water right associated with this well
        nWR=0
        iplan=0
        ipn=0
        do i=1,10
...
c		Get every plan associated with this well
            ...
            if(iplan.eq.0) then
              ...
              if(ipn.eq.0) then
                ...
                nwellP(k)=ipn                  
                cwellP(iplan)= pid(ipX)
c               cwellP(iplan)= adjustR(cwellP(ipX))
              endif
            endif  
c            
c ---------------------------------------------------------
...
c               Print title card every year
        if(iout.eq.1) write(nlog,*) '  OutPlnW; iplan = ', iplan
        if(iplan.gt.0) write(nout,277)
        do i=1,iplan
          write(nout,278) i, cwellp(i), NameP(i)
        end do          
c
c _________________________________________________________
c  
...
c
c               Print title card every year
c         write(nout,230) (adjustR(cwellp(i)),i=1,10), (i, i=1,12) 
...
c
            ...
            read(42,rec=irecs,err=310) (dat1(i),i=1,ndivw)
            if(iout.eq.1) write(nlog,*) ' OutPlnW; ',
...
c _________________________________________________________
c
c               14c; Print Average Monthly basin total
        cdx='Basin Total '
c
c ---------------------------------------------------------
c               Print title card 
...
c 210 FORMAT('',/, '   Well Water Only Summary ',a5,/,3x,a80,/,
  ...
  222 format(i5, i5,1x,a12, i5,1x,a12, i5, i5, i5, 1x, a12) 
  ...
  228 format(/, 
     1 '  OutPlnW; Warning Well ID = ', a12, ' is not tied to a plan',/
     1 '           But pumping = ', f8.0, ' in ', i5)
 
  230   format(/,                                            
     1  '                                           From',
     1  11('    From Plan'),/
     1  'Structure   River                          Well',
     1  10(1x,a12), '        Total',/
     1  'ID          ID           Year   Mo           NA',
     1  10('          (+)'),'           NA',/
     1  34x, 12('         (', i2,')'),/
     1  2('___________ '), ' ____ ____', 12(' ____________'))
...
c               Error messages
  ...
  302  format(/, 72('_'),/
     1  '   OutPlnW; Problem dimension exceeded = ', i5)
       goto 400
     
  310  write(nlog,312)
  312  format(/, 72('_'),/
     1  '   OutPlnW; Requested data exceeds binary file size')
       goto 400
c
c               Error Warning
  400 write(6,410) 
      write(nlog,420) 
      call flush(6)
  410 format('    Stopped in OutPlnW',/,
     1       '    See the *.log file')
  420 format('    Stopped in OutPlnW')
      write(6,*) 'Stop 1'
      call flush(6)
      call exit(1)
