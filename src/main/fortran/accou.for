c accou - distributes a reservoir right to individual accounts
...
c
        subroutine accou(maxacc,maxown,nr,ownmon,curown,accr,ia,
     1    ownmax, iown, nrown1,cursa, actaf, iresty1, icx, cresid1)
c
...
c               Distribute to one account
        if(iresty1.le.-1) then
          ...
          accr(ia,n1)=accr(ia,n1)+c
          vola = vola + amax1(0.0, ownmax(n1)-curown(n1))
          if(iout.eq.1) write(nlog,*)' Accou; ', n1, curown(n1)
        endif
...
c               Formats
 100  format(/
     1 '  Accou; Distributes a diversion to accounts',/
 ...
 200  format(/,
     1 'Accou; ', a20, ' allocating a right to owners ',/
...
c               Error Processing
 ...
 9999 write(6,*)  '  Stopped in Accou, see the log file (*.log)'
      write(99,*) '  Stopped in Accou'
      write(6,*) 'Stop 1'
      call flush(6)
      call exit(1)
      stop 
