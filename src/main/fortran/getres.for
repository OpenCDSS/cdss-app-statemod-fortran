c getres - reads in reservoir data
...
c                       + print detailed information (GetRes)
...
cx      dimension xmon(12)
C
c
c rrb 2021/04/18; Compiler warning
cx    character ch3*3, ch4*4, blank*12, crtnid*12, xmon*4,
...
cx280 write(nlog,103)
      ...
      write(6,103)
  103   format(/,
     1  '  GetRes; Reservoir Station File (*.res) ')
      iin2=iin
      ...
      if(infile.eq.1) then
        ...
        rec256=fileName(ifn)
        filena=rec256(1:72)
c       write(nlog,*) ' GetRes; fileName = ', fileName(ifn)
c       write(nlog,*) ' GetRes; filena   = ', filena
      else   
        filena=filenc
...
C       
  300   IRSSTA(NR)=IS
        if(iout.eq.1) write(nlog,*) '  GetRes; nr, is ', nr, is
c
...
C
     ...
     1    ' Getres; Reading ownership data', ji, je
        DO 320 J=JI,JE
c
c rrb 01/10/95; 
...
c		Read evap or net evap data
      ...
      if(iout.eq.1) write(nlog,*) 
     1  '  GetRes; Res Evaporation read; ', ji,je
...
c       write(nlog,*) '  GetRes;', nr, conten(irg,nr), suarea(irg,nr)
  380 continue
c
...
c       write(nlog,*) '  GetRes;', nr, nevapo(nr), nevapo(nr+1)
      end do
C
      NPRECP(1)=1
      DO NR=1,NUMRES
...
c               Open file
      ...
      if(filenaR(1:2).ne.'-1') then
        ...
        write(6,104)
  104   format(/,
     1  '  GetRes; Reservoir Station Return File (*.rrf) ')
      
        inR=55
        write(nlog,940) 'Reservoir Return Flow (*.rrf) ', filenaR
c       write(nlog,*) '  GetRes; inR', inR
...
c                       Return Flow data
      ...
      if(inR.gt.0) then
        ...
        if(iout.eq.2)
     1    write(nlog,*) '  GetRes; Calling getRtnX for Res. Returns'
c
c _________________________________________________________
c
...
c               istrTyp=2 for a reservoir
     ...
     1    maxrtnRP,  maxsta,   maxres,   nstrtn,   in1,
     1    inR,       nw,       ji,       je,       iin2,
     1    numsta,    ityp,     iloss,    interv,   pcttotRP,
     1    pctlosRP,  irtndlRP, irnordRP, irnstaRP, numrtnRP,  
     1    nrtnRP,    numRES,   istrtn,   ndnnod,   idncod,  
     1    cgoto,     ResNam1,  cresid,   cstaid,   cstadn, 
     1    filenaR)
       endif
c
...
cxc          write(nlog,*) '  GetRes; nr, is, nstrtn, irnord(is), cstaid' 
cxc          write(nlog,*) '  GetRes;',iprintr2, is, nstrtn, irnord(is),
cxc     1      cstaid(is)
cx        endif
cx      end do
cxc
...
c _________________________________________________________
  ...
  927 format(/,
     1  ' GetRes; Problem. End of file # ', i4, ' encountered',/,
     1  '         File name: ', a256)
      goto 9999
c
  928 write(nlog,929) iin2, filena
  929 format(/,
     1' GetRes; Problem reading file # ', i4,/,
     1       '   File name: ', a256,/
 ...
 1420 format(/,
     1 '  GetRes; Problem opening file: ', a256)
      goto 9999
...
cx     1  ' recommend you revise the common block size')
 ...
 1281  FORMAT(/,
     1  '  GetRes; Warning See *.chk for details regarding: ',a24)

 1310  FORMAT(a12,a24,a12,i8,f8.0,1x,a12)
 
 1402  format(/,72('_'),/
     1 '  GetRes; Problem res. ID ', a12, ' has a daily ID = ', a12,/
...
cx 1430 format(/,
cx     1 '  GetRes; Reservoir Return Flow Details',/
...
c _________________________________________________________
 ...
 9999 write(6,9440) 
      ...
      call flush(6)
 9440 format(/,72('_'),/
     1  ' GetRes; Stopped in GetRes, see the log file (*.log)')
 9450 format('  Stopped in GetRes')
      write(6,*) 'Stop 1'
      call flush(6)
      call exit(1)
