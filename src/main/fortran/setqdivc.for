c setqdivc - It sets qdiv data for carriers and return to river.
c            It also calculates return flows from any losses.
...
c	
        ...
        dimension intern(maxopr,20), internT(maxopr,20)
        dimension pctlosPP(maxrtnPP)
        dimension rlossP(maxPlan)
        dimension oprlossC(maxopr,20)
...
c		Step 5;	Set Releases back to the River
          ...
          if(internT(l2,i).eq.2) then
            ...
            nlast=0
c           write(nlog,*) ' SetQdivc; qdiv(36', i, l2,inode,divactC*fac
          endif  
c
c
c ---------------------------------------------------------
...
c		Formats        
 200   format(
     ...
     1 '     idcdX     idcdC   divactX  TranLoss   oprEff1   divactC',
